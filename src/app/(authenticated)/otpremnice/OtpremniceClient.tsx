...
export default function OtpremniceClient({
  initialDeliveries,
}: {
  ...
  const handleDelete = async () => {
    ...
    const res = await fetch(`/api/deliveries/${deleteTarget.id}`, {
      method: "DELETE",
    });
    ...
    toast.success(`Otpremnica br. ${deleteTarget.number} obrisana`);
    setDeleteTarget(null);
    setDeleteLoading(false);
    router.refresh();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {deliveries.length}{" "}
            {deliveries.length === 1 ? "otpremnica" : "otpremnica"}
          </p>
        </div>
        <Link
          href="/otpremnice/nova"
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1 justify-end">
                        <button
                          onClick={() => router.push(`/otpremnice/${d.id}`)}
      ...
      <Modal
        ...
        title="Obrisi otpremnicu"
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-700">
            Da li ste sigurni da zelite da obrisete otpremnicu br.{" "}
            <strong>{deleteTarget?.number}</strong>?
          </p>
          {deleteTarget?.affectsStock && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
              <p className="text-sm text-yellow-800">
                Ova otpremnica je uticala na magacin. Brisanjem ce se vratiti
                kolicine proizvoda.
              </p>
            </div>
          )}
